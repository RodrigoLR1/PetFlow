...
function startBossFight(type){
    ...
    else if(type==='tatu'){boss.y=450;boss.w=96;boss.h=96;boss.state='quicando';boss.dx=9;boss.dy=-9;boss.phaseTimer=350;boss.powerUpTimer=150;bossIntroName='🦔 TATU-PEIXEIRA — Escudo de Espinhos!';if(bossBarLabel)bossBarLabel.textContent='🦔 TATU-PEIXEIRA';}
...
function processarMorteDoBoss(){
    ...
    bonusPoints+=Math.max(0,Math.floor(300-(Date.now()-boss.fightStartTime)/1000*3));
    addScreenShake(14);playSound('bossDie');
    for(let i=0;i<5;i++)setTimeout(()=>{spawnParticles(boss.x+boss.w/2+(Math.random()-.5)*80,boss.y+boss.h/2+(Math.random()-.5)*50,'#e74c3c',12,true);spawnParticles(boss.x+boss.w/2+(Math.random()-.5)*60,boss.y+boss.h/2+(Math.random()-.5)*40,'#f39c12',10,true);},i*130);
}
...
function onTatuHitWall(){
    if(boss.hp<=boss.maxHp/2&&boss.state==='quicando'){
        ...
        for(let j=0;j<n;j++) boss.projectiles.push({x:boss.x+boss.w/2-36,y:boss.y+boss.h/2-36,w:72,h:72,dx:(Math.random()-.5)*18,dy:-9-Math.random()*9,type:'espinho'});
...
function updateTatuBoss(){
    ...
    if(boss.state==='quicando'){
        ...
        if(hw){if(boss.x<=cameraX||boss.x+boss.w>=cameraX+1000)boss.dx*=-1;if(boss.y<=0||boss.y+boss.h>=550)boss.dy*=-1;if(Math.random()<0.5){const al=players.find(p=>p.active&&!p.dead)||players[0];const dxJ=al.x-(boss.x+boss.w/2),dyJ=al.y-(boss.y+boss.h/2),dist=Math.sqrt(dxJ*dxJ+dyJ*dyJ);if(dist>0){boss.dx=(dxJ/dist)*13;boss.dy=(dyJ/dist)*13;if(boss.y>=550-boss.h&&boss.dy>-2)boss.dy=-9;if(boss.y<=0&&boss.dy<2)boss.dy=9;if(boss.x<=cameraX&&boss.dx<2)boss.dx=9;if(boss.x+boss.w>=cameraX+1000&&boss.dx>-2)boss.dx=-9;}}onTatuHitWall();}
    ...
    }else if(boss.state==='cansado'){boss.frameCount++;if(boss.frameCount>10){boss.frameIndex=(boss.frameIndex+1)%5;boss.frameCount=0;}boss.phaseTimer--;if(boss.phaseTimer<=0){boss.state='quicando';boss.phaseTimer=350;const al=players.find(p=>p.active&&!p.dead)||players[0];if(Math.random()<0.5){const dxJ=al.x-boss.x,dyJ=al.y-boss.y,dist=Math.sqrt(dxJ*dxJ+dyJ*dyJ);if(dist>0){boss.dx=(dxJ/dist)*12;boss.dy=-10;}}else{boss.dx=9*(Math.random()>.5?1:-1);boss.dy=-10;}}}
}
...
function updateBoss(){
    ...
    if(boss.state==='dying'){boss.deathTimer--;if(boss.deathTimer<=0){boss.active=false;bossHealthBar.classList.add('hidden');stopMusic();
        if(boss.type==='carcará'){currentStage=2;cameraLocked=false;nextChunkX=cameraX+1200;reviveAllPlayers();worldTransitionTimer=220;worldTransitionText='🌧️ Mundo 2 — Caatinga na Chuva';playSound('worldNext');setTimeout(()=>startMusic('explore'),600);}
                ...
                saveHighScore();
                _endScore = score; _endTime = totalTimeSeconds;
...
function drawBoss(){
    ...
    for(let i=boss.projectiles.length-1;i>=0;i--){
        const bp=boss.projectiles[i];bp.x+=bp.dx;bp.y+=bp.dy;if(bp.type==='espinho')bp.dy+=0.42;
...
class Player {
    ...
    collectPowerUp(type){
        ...
        if(type==='life'){if(this.lives<3){this.lives++;playSound('lifeup');}else bonusPoints+=250;}
        bonusPoints+=100;playSound('powerup');
...
function gameLoop(){
    ...
    if(gameState==='PLAYING'){
        ...
        if(alive.length===0){
            // Captura ANTES de qualquer reset — fix bug ranking
            _endScore = score; _endTime = totalTimeSeconds;
        ...
        if(boss.active){
            updateBoss();drawBoss();
            if(boss.state!=='dying'){players.forEach(p=>{if(!p.active||p.dead||!isColliding(p.hitbox,boss))return;if(boss.state==='escondida'||boss.state==='pulo_fundo')return;let canDmg=false;if(boss.type==='carcará'&&p.dy>0&&p.y+p.h<boss.y+60)canDmg=true;if(boss.type==='tatu'&&boss.state==='cansado'&&p.dy>0&&p.y+p.h<boss.y+34)canDmg=true;if(boss.type==='sussuarana'&&p.dy>0&&p.y+p.h<boss.y+44&&!['bote','bote_arbusto'].includes(boss.state))canDmg=true;if(canDmg){p.dy=-16;boss.hp--;bossHealthFill.style.width=`${(boss.hp/boss.maxHp)*100}%`;addScreenShake(5);playSound('bossHit');boss.hitFlash=6;spawnParticles(boss.x+boss.w/2,boss.y+boss.h/2,'#e74c3c',7);if(boss.hp<=0)processarMorteDoBoss();}else{if(boss.type==='tatu'&&boss.state==='cansado'){if(p.x<boss.x)p.x-=6;else p.x+=6;}else p.takeDamage();}});}
        }
    ...
    // ── OVERLAYS FINAIS ────────────────────────────────────
    if(gameState==='GAMEOVER'){gameOverScreen.classList.remove('hidden');finalScoreTxt.innerHTML=`Pontuação: <strong>${score.toLocaleString('pt-BR')}</strong><br>Tempo: ${formatTime(totalTimeSeconds)}<br><small style="opacity:0.55">🏆 Recorde: ${highScore.toLocaleString('pt-BR')} pts</small>`;}
    if(gameState==='VICTORY'){victoryScreen.classList.remove('hidden');victoryScoreTxt.innerHTML=`Pontuação Final: <strong>${(score+2000).toLocaleString('pt-BR')}</strong><br>Tempo: ${formatTime(totalTimeSeconds)}<br><small style="opacity:0.55">🏆 Recorde: ${highScore.toLocaleString('pt-BR')} pts</small>`;}
}
